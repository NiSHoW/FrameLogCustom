...
namespace FrameLog.Translation
{
    public partial class ValueTranslationManager : ISerializationManager, IBindManager
    {
        ...
        public virtual Boolean Compare(object oldValue, object newValue)
        {
            if ((oldValue ?? newValue) != null) { 
                var type = (oldValue ?? newValue).GetType();
                foreach (var comparetor in translators.OfType<IComparator>())
                {
                    if (comparetor.Supports(type))
                        return comparetor.Compare(oldValue, newValue);
                }
            }
